document.querySelector(".button").addEventListener("click", function () {
    ...
    let diffMonths = Math.ceil(diffTime / (1000 * 60 * 60 * 24 * 30)); // Aproximado para meses
 
    // Limitando o número de meses a 12, para considerar apenas o último ano
    diffMonths = Math.min(diffMonths, 12);
 
    // Cálculo de férias e décimo terceiro proporcionais
    const feriasProporcionais = ((salarioBruto / 12) * diffMonths) +((salarioBruto / 12) * diffMonths)/3 ;
